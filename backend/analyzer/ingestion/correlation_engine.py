...
def run_correlation():
    ...
    for log in logs:
        ...
        log["dt"] = _normalize_timestamp(timestamp)
    ...
    for machine, machine_logs in machines.items():
        ...
        for key, failed_logs in failed_groups.items():
            ...
            #
            # IMPORTANT:
            # parser.py maps Event ID 4624 to
            # LOGON_SUCCESS
            # ---------------------------------------------
            ...
            for log in machine_logs:
                ...
                # FIXED:
                # LOGON_SUCCESS follows failed login attempts.

                if log["event_type"] != "LOGON_SUCCESS":
                    continue
        ...
        for i in range(
            ...
            # ---------------------------------------------
            # Stage A
            # Successful Login → Admin Group Modification
            # ---------------------------------------------

            # Windows-compatible:
            # LOGON_SUCCESS followed by ADMIN_GROUP_ADDED.
            ...
            # ---------------------------------------------
            # Collect Every Admin Group Change
            # ---------------------------------------------
        ...
        for i in range(len(machine_logs)):
            ...
            for j in range(i + 1, len(machine_logs)):
                ...
                # Ransomware detection uses a longer observation
                # window because file encryption/access activity
                # can continue for several minutes.
                if nxt["dt"] - current["dt"] > timedelta(minutes=10):
                    break
